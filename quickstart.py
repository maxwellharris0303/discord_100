...
SAMPLE_SPREADSHEET_ID = '1fmjCNDikGXq0uq5leFD8RqBFUEDBWY4kHZNM4GqzGsI'
...
def main():
    ...
    try:
        ...
        for row in values:
            ...
            if len(row) >= 1:
                apn_list.append('%s' % (row[0]))
            if len(row) >= 3:
                taxes_list.append('%s' % (row[2]))
        print(getLastIndex())
...
def getColumnCount():
    columnCount = len(apn_list)
    return columnCount
def getLastIndex():
    return len(taxes_list)

def getAPNList():
    return apn_list

def insert_data(data_range, data):
